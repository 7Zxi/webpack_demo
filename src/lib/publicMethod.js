  ...
  var _publicMethod = {
    ...
    pageAdapter: function (size) {
      ...
      function setHtmlSize() {
        ...
        htmlEl.style.fontSize = (value / size) * 100 + "px";
      ...
      function fixException() {
        var clientWidth = document.documentElement.clientWidth;
        var initSize = 100 * (clientWidth / size);
        if (window.getComputedStyle(htmlEl).fontSize) {
          let size = window.getComputedStyle(htmlEl).fontSize.split('p')[0];
          if (size > initSize) {
            const newSize = initSize / size * initSize;
      ...
      function editNum(domNum, timeNum) {
        var timer = setTimeout(function () {
          ...
          if (domNum === cacheArr[0]) {
            ...
            clearTimeout(timer);
            isEdit = false;
            (cacheArr[0] || cacheArr[0] === 0) && insertNum();
          }
      ...
      function shareAndConfig() {
        if (config) {
          wx.config({
            ...
            jsApiList: ['updateTimelineShareData', 'updateAppMessageShareData'].concat(config.jsApiList || []) // 必填，需要使用的JS接口列表
          })
        }
        wx.ready(function () {
    ...
    addMusic: function (params) {
      ...
      document.addEventListener("WeixinJSBridgeReady", function () {
        audio.play();
        img.classList.remove('music_pause');
    ...
    animateCSS: function (node, animationName, callback) {
      return new Promise(resolve => {
        ...
        animationName.push('animated', 'animate__animated');
    ...
    //移动端:ios输入框弹出软键盘影响定位处理、安卓输入框弹出遮盖视图
    insertStyle: function () {
      ...
      function showView(eventObj) {
        eventObj.addEventListener('focus', function () {
          let u = navigator.userAgent, app = navigator.appVersion;
          let isAndroid = u.indexOf('Android') > -1 || app.indexOf('Linux') > -1;
          if (isAndroid) {
            setTimeout(function () {
    ...
    // 添加摇一摇事件
    addShakeEvent: function (empty_value = 100, thread_hold = 13.8) {
      var EMPTY_VALUE = empty_value;
      // 阈值，可以调整大小来触发摇一摇的简易程度
      var THREAD_HOLD = thread_hold;
